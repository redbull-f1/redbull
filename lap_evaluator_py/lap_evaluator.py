...
class LapEvaluator(Node):
    def __init__(self):
        ...
        self.d_errors = []
        self.lap_times = []
        self.lap_rmse_list = []  # 각 랩의 RMSE 저장
        self.latest_d = 0.0
    ...
    def odom_callback(self, msg):
        ...
        if self.prev_s > 5.0 and s < 2.0:  # Thresholds may need tuning
            ...
            self.lap_rmse_list.append(rmse_d)
            self.get_logger().info(f"Lap {self.lap_count}: Time = {lap_time:.2f}s, Max d = {max_d:.3f}, RMSE d = {rmse_d:.3f}")
            # 11번째 랩 종료 시 2~11랩의 RMSE 평균 출력
            if self.lap_count == 11 and len(self.lap_rmse_list) >= 11:
                avg_rmse = np.mean(self.lap_rmse_list[1:11])  # 2~11랩
                self.get_logger().info(f"[Lap 2~11] RMSE tracking error 평균: {avg_rmse:.3f}")
            # Reset for next lap
            self.lap_start_time = now
            self.d_errors = [d]
        else:
