...
# tracking.py adapting the Kalman Filter.
# ------------------------------------------------------------------------------
...
class Opponent_state:
    ...
    def __init__(self):
        ...
        self.dynamic_kf = EKF(dim_x=4, dim_z=4)
    ...
    def target_velocity(self) :
        # idx_closest_waypoint= np.argmin(np.abs(np.array([normalize_s(wpnt.s_m,Opponent_state.track_length) for wpnt in Opponent_state.waypoints]) - self.dynamic_kf.x[0]))
    ...
    def update(self, tracked_obstacle: ObstacleSD):
        ...
        vs = tracked_obstacle.measurments_vs[-1]
        ...
        z = np.array([
            ...
            tracked_obstacle.measurments_vs[-1],
            tracked_obstacle.measurments_d[-1],
            tracked_obstacle.measurments_vd[-1],
        ])
...
class ObstacleSD:
    ...
    def __init__(self, id, s_meas, d_meas,vs_meas, vd_meas, lap, size, isVisible):
        """
        ...
        self.measurments_vs = [vs_meas]
        self.measurments_vd = [vd_meas]
        self.mean = [s_meas,d_meas] # [mean_s. mean_d] 
        self.mean_v = [vs_meas,vd_meas]
        self.static_count = 0
        self.total_count = 0
        self.nb_meas = 0
    ...
    def update_mean(self,track_length):
        if (self.nb_meas == 0):
            ...
            self.mean_v = [self.measurments_vs[-1],self.measurments_vd[-1]]
        else:
            # ------------------------------------------------------------------------------------
            ...
            self.mean_v[1] = (self.mean_v[1]*self.nb_meas+self.measurments_vd[-1])/(self.nb_meas+1)
            # self.mean[1] = self.measurments_d[-1]
            ...
            #self.mean[0] = self.measurments_s[-1]

    def std_s (self,track_length):
        sum = 0
        # considered_s = self.measurments_s[-10:]
        mean_s = self.mean[0]
...
class StaticDynamic:
    ...
    It subscribes to the following topics:
        - `/obacles_centerspeed`: Publishes the raw obstacle data.
    ...
    def __init__(self):
        ...
        Opponent_state.measurment_var_s = rospy.get_param("/tracking/measurement_var_s")
        Opponent_state.measurment_var_d = rospy.get_param("/tracking/measurement_var_d")
        Opponent_state.measurment_var_vs = rospy.get_param("/tracking/measurement_var_vs")
        Opponent_state.measurment_var_vd = rospy.get_param("/tracking/measurement_var_vd")
    ...
    def shutdown(self):
        rospy.logwarn('StaticDynamic is shutdown')
    ...
    def update_tracked_obstacle(self, tracked_obstacle: ObstacleSD, meas_obstacle):
        ...
        tracked_obstacle.measurments_vs.append(meas_obstacle.vs)
        tracked_obstacle.measurments_vd.append(meas_obstacle.vd)
        # handle list lenght
    ...
    def calc_distance_obs_car(self, tracked_obstacle, car_s):
        ...
        # not jump from 0 to track_length immediatly
        # if(distance_obs_car > self.track_length/4):
        #     distance_obs_car = -car_s + tracked_obstacle.measurments_s[-1]

        # for absolute dist to obs
        # if(distance_obs_car > self.track_length/4):
        #     distance_obs_car = (car_s - tracked_obstacle.measurments_s[-1]) % self.track_length
    ...
    def check_in_field_of_view(self, vec_car_to_obs, car_orientation_copy, dist_to_obs) -> bool:
        ...
        # not delete objects that are behind us
        # if idx < 179 or idx > 899: return False
        # lidar_scan = self.scans[idx]
        ...
        # if (dist_to_obs < self.scans[idx-4] and dist_to_obs < lidar_scan and dist_to_obs < self.scans[idx+4]):
        #     return True
        return False
    ...
    def update(self):
        ...
        for tracked_obstacle in self.tracked_obstacles:
            ...
            if isTracked:
                #print(f"found gnn: {tracked_obstacle.id}")
                ...
                if tracked_obstacle.ttl <= 0:
                    ...
                    # if (tracked_obstacle.staticFlag and distance_obstacle_car < self.dist_deletion):
                        ...
                        if (self.check_in_field_of_view(vec_car_to_obs, car_orientation_copy, distance_obstacle_car)):
                            # print("ttl decreased due to not seen: ", tracked_obstacle.id)
                            tracked_obstacle.ttl-=1
                            tracked_obstacle.isVisible = True
                        else:
        ...
        for meas_obstacle in meas_obstacles_copy:
            ...
            self.tracked_obstacles.append(ObstacleSD(
                ...
                vs_meas = meas_obstacle.vs,
                vd_meas = meas_obstacle.vd,
    ...
    def publishObstacles (self):
        ...
        if self.opponent_obstacle.isInitialised:
            if self.opponent_obstacle.dynamic_kf.P[0][0]<self.var_pub:
                ...
                obs_msg.vs    = self.opponent_obstacle.dynamic_kf.x[1]
                obs_msg.vd    = self.opponent_obstacle.dynamic_kf.x[3]
    ...
    def main (self):
        ...
        rospy.loginfo('[Perception Classifier]: Waiting for global wpnts')
        rospy.wait_for_message('/global_waypoints', WpntArray)
        ...
        rospy.loginfo('[Perception Tracking]: Ready')
