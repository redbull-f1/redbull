...
- Kalman CV tracking (map frame)
- /obstacles: redbull_msgs/ObstacleArray (frame_id=map, x/y/vx/vy/yaw 모두 map 기준)
- RViz Markers:
  * /objects_map   (frame_id=map)
  * /objects_laser (frame_id=laser)
"""
...
# -------------------- Kalman Track (CV on map frame) --------------------
class KalmanTrack:
    """
    Constant-Velocity KF on map frame: state = [x, y, vx, vy]^T
    """
    __slots__ = ("id", "x", "P", "hits", "missed", "confirmed", "history",
                 "yaw", "ema_alpha", "sigma_a", "r_pos", "r_vel")

    def __init__(self, tid: int, x_init: np.ndarray, yaw_init: float,
                 ema_alpha: float, sigma_a: float, r_pos: float, r_vel: float):
        self.id = tid
        self.x = x_init.reshape(4, 1).astype(np.float32)  # [x, y, vx, vy]^T
        # 초기 공분산(위치/속도 1)
        self.P = np.diag([1.0, 1.0, 1.0, 1.0]).astype(np.float32)
        self.hits = 1
        self.missed = 0
        self.confirmed = False
        self.history = deque(maxlen=5)

        self.yaw = float(yaw_init)
        self.ema_alpha = float(ema_alpha)
        self.sigma_a = float(sigma_a)  # 가속도 표준편차 [m/s^2]
        self.r_pos = float(r_pos)      # 위치 측정 표준편차 [m]
        self.r_vel = float(r_vel)      # 속도 측정 표준편차 [m/s]
    ...
    def predict(self, dt: float):
        ...
        # 속도기반 yaw 예측 (관측 없을 때 보정용)
        vx, vy = float(self.x[2,0]), float(self.x[3,0])
        if abs(vx) + abs(vy) > 1e-3:
            yaw_pred = math.atan2(vy, vx)
            # 부드럽게만 반영 (관측 시 update에서 재보정)
            self.yaw = wrap_angle((1.0 - self.ema_alpha) * self.yaw + self.ema_alpha * yaw_pred)

    def update(self, z: np.ndarray, yaw_meas: float | None):
        # H = I (x,y,vx,vy 모두 관측 가능)
        H = np.eye(4, dtype=np.float32)
        R = np.diag([self.r_pos**2, self.r_pos**2, self.r_vel**2, self.r_vel**2]).astype(np.float32)
        ...
        if yaw_meas is not None:
            err = wrap_angle(yaw_meas - self.yaw)
            self.yaw = wrap_angle(self.yaw + self.ema_alpha * err)
...
class SimDetector(Node):
    def __init__(self):
        ...
        self.declare_parameter('model_path', '/home/harry/ros2_ws/src/TinyCenterSpeed/src/pt/CenterSpeedDense_dynamic99_20250820_04%_epoch_41.pt')
        # self.declare_parameter('model_path', '/home/harry/ros2_ws/src/TinyCenterSpeed/src/pt/0_best_objfree_trainfree52497_20250818_170557_epoch_14_loss_088283.pt')

        self.declare_parameter('image_size', 128)
        self.declare_parameter('dense', True)
        self.declare_parameter('num_opponents', 1)
        self.declare_parameter('detection_threshold', 0.80)
        ...
        # Tracker params
        self.declare_parameter('gate_dist', 0.35)          # 같은 물체로 인정할 최대 거리[m]
        self.declare_parameter('min_hits', 3)              # 확정까지 필요한 연속 관측 수
        self.declare_parameter('max_missed', 3)            # 끊겨도 유지할 프레임 수
        self.declare_parameter('ema_alpha', 0.25)          # yaw/merge용 EMA 계수
        self.declare_parameter('merge_dist', 1.0)          # 근접 중복 트랙 병합 임계[m]
        ...
        # --------- Kalman Filter params (map frame CV) ---------
        self.declare_parameter('kf_sigma_a', 2.0)         # [m/s^2] 가속도 표준편차
        self.declare_parameter('kf_r_pos', 0.15)          # [m]     위치 측정 표준편차
        self.declare_parameter('kf_r_vel', 0.8)           # [m/s]   속도 측정 표준편차
        self.declare_parameter('gate_mahalanobis2', 16.0) # (옵션) 사용 X, 필요한 경우 확장
        self.declare_parameter('dt_fallback', 1.0/40.0)   # 스캔 40Hz 기본
        ...
        self.kf_sigma_a          = float(self.get_parameter('kf_sigma_a').value)
        self.kf_r_pos            = float(self.get_parameter('kf_r_pos').value)
        self.kf_r_vel            = float(self.get_parameter('kf_r_vel').value)
        self.gate_m2             = float(self.get_parameter('gate_mahalanobis2').value)
        self.dt_fallback         = float(self.get_parameter('dt_fallback').value)
        ...
        try:
            from CenterSpeed import CenterSpeedDense, CenterSpeedDenseBottleneck
        ...
        # self.net = CenterSpeedDenseBottleneck(input_channels=4, image_size=self.image_size)
    ...
    def _calc_dt(self, scan_msg: LaserScan) -> float:
        if scan_msg is None or scan_msg.header is None:
            return self.dt_fallback
        now = self._stamp_to_sec(scan_msg.header.stamp)
        if self._last_scan_time is None:
            self._last_scan_time = now
            return self.dt_fallback
        dt = max(1e-3, now - self._last_scan_time)
        self._last_scan_time = now
        return dt
    ...
    def associate(self, tracks, meas_map):
        """Greedy 최근접 매칭(1:1) + 거리 게이팅 (map frame)
           tracks: KalmanTrack list (이미 predict 된 상태)
           meas_map: list of (x_m, y_m, yaw_m, vx_m, vy_m)
        """
    ...
    def merge_duplicates(self):
        ...
        for i in range(n):
            ...
            for j in range(i + 1, n):
                ...
                if euclidean(ti.pos, tj.pos) < self.merge_dist:
                    # keep: hits 많은 쪽(동률이면 공분산 trace 작은 쪽)
                    trace_i = float(np.trace(ti.P))
                    trace_j = float(np.trace(tj.P))
                    if (ti.hits > tj.hits) or (ti.hits == tj.hits and trace_i <= trace_j):
                        keep, drop = ti, tj
                        drop_idx = j
                    else:
                        keep, drop = tj, ti
                        drop_idx = i
                    # 상태를 약간 섞어 안정화 (가중 EMA)
                    w_keep = keep.hits + 1e-6
                    w_drop = drop.hits + 1e-6
                    alpha = self.ema_alpha * (w_drop / (w_keep + w_drop))
                    keep.x = (1 - alpha) * keep.x + alpha * drop.x
                    # yaw도 부드럽게
                    yaw_err = wrap_angle(drop.yaw - keep.yaw)
                    keep.yaw = wrap_angle(keep.yaw + alpha * yaw_err)
    ...
    def process(self):
        ...
        # ---------- 측정 구성 ----------
        meas_lidar = []   # (x_l, y_l, yaw_l, vx_l, vy_l)  # (디버그용)
        ...
        # ---------- Kalman Tracking ----------
        # 0) dt 및 모든 트랙 예측
        dt = self._calc_dt(self.scan_data)
        for t in self.tracks:
            t.predict(dt)

        # 1) 연관(associate) - 예측 위치 vs 측정 위치
        ...
        for ti, mi in matches:
            ...
            z = np.array([mx, my, mvx, mvy], dtype=np.float32)
            t.update(z, yaw_meas=myaw)
        ...
        for mi in unmatched_meas_idx:
            mx, my, myaw, mvx, mvy = meas_map[mi]
            x_init = np.array([mx, my, mvx, mvy], dtype=np.float32)
            t = KalmanTrack(
                tid=self.next_id,
                x_init=x_init,
                yaw_init=myaw,
                ema_alpha=self.ema_alpha,
                sigma_a=self.kf_sigma_a,
                r_pos=self.kf_r_pos,
                r_vel=self.kf_r_vel
            )
            self.next_id += 1
        ...
        for t in publishables:
            ...
            ob.x   = float(x_m)
            ob.y   = float(y_m)
            ob.vx  = float(vx_m)   # KF 추정 속도
            ob.vy  = float(vy_m)   # KF 추정 속도
        ...
        if self.publish_markers:
            ...
            if self.odom is not None:
                ...
                for t in publishables:
                    x_m, y_m = t.pos
                    x_l, y_l = self.map_to_lidar(x_m, y_m, self.odom)
                    m = Marker()
